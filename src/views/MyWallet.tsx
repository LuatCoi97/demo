...
const MyWallet: React.FC = () => {
    ...
    let walletAddress = "";
    let AuctionAddress = "3EERzZ6dHvYKksjDuUECDTRNKENCw8NcN4LFFKj4C6th";
    ...
    function getShow() {
        ...
        show({ env: 'devnet', keypair: Keypair.generate().secretKey, auctionHouse: AuctionAddress }).then((x: { auctionHouseKey: string; treasuryMint: { toBase58: () => string; }; authority: { toBase58: () => string; }; creator: { toBase58: () => string; }; auctionHouseFeeAccount: { toBase58: () => string; }; auctionHouseTreasury: { toBase58: () => string; }; feeWithdrawalDestination: { toBase58: () => string; }; treasuryWithdrawalDestination: { toBase58: () => string; }; sellerFeeBasisPoints: string; requiresSignOff: string; canChangeSalePrice: string; bump: string; feePayerBump: string; treasuryBump: string; }) => {
    ...
    function getCreateauctionhouse() {
        create_auction_house({ env: 'devnet', sfbp: 1000, ccsp: false, rso: false, keypair: Keypair.generate().secretKey }).then((x: string) => {
    ...
    function getSell() {
        ...
        sell({ auctionHouse: AuctionAddress, buyPrice: price, mint: mintAddress, tokenSize: '1', env: 'devnet', keypair: Keypair.generate().secretKey }).then((x: { account: string; mintAddress: string; price: string; }) => {
    ...
    function getBuy() {
        buy({ vauctionHouse: AuctionAddress, buyPrice: price, tokenSize: '1', mint: mintAddress, env: 'devnet', keypair: Keypair.generate().secretKey }).then((x: string) => {
