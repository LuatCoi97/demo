...
export const withdraw = async (cmd : any) => {
    ...
    if (auctionHouseKeypairLoaded) {
      ...
      instruction.keys
        .filter((k: { pubkey: { equals: (arg0: web3.PublicKey) => any; }; }) => k.pubkey.equals(auctionHouseKeypairLoaded.publicKey))
        .map((k: { isSigner: boolean; }) => (k.isSigner = true));
    }

    instruction.keys
      .filter((k: { pubkey: { equals: (arg0: web3.PublicKey) => any; }; }) => k.pubkey.equals(walletKeyPair.publicKey))
      .map((k: { isSigner: boolean; }) => (k.isSigner = true));
...
export const sell = async (cmd : any) => {
    ...
    if (auctionHouseKeypairLoaded) {
      ...
      instruction.keys
        .filter((k: { pubkey: { equals: (arg0: any) => any; }; }) => k.pubkey.equals(auctionHouseKeypairLoaded.publicKey))
        .map((k: { isSigner: boolean; }) => (k.isSigner = true));
    }
...
// @ts-ignore: Object is possibly 'null'.
      const a = new web3.PublicKey(metadataDecoded.data.creators[i].address)
...
export const buy = async (cmd : any ) => {
    ...
    if (!isNative) {
      instruction.keys
        .filter((k: { pubkey: { equals: (arg0: web3.PublicKey) => any; }; }) => k.pubkey.equals(transferAuthority.publicKey))
        .map((k: { isSigner: boolean; }) => (k.isSigner = true));
    }
...
export const deposit = async (cmd : any) => {
    ...
    const output = {'deposited': amountAdjusted,'account':auctionHouse,'newBalance':currBal + amountAdjusted}
